...
//
// The core recommendation pipeline, taking a userId directly rather
// than a JWT — this is what lets both `recommend` (resolves userId
// from the caller's session) and `notify-recommendations` (iterates
// real user ids on a schedule, no request/session involved) share the
// exact same logic instead of drifting apart over time.

import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';
import { generateDna, INTENSITY_KEYS } from './dna.ts';
...
export async function runRecommendationPipeline(
  ...
  const excluded = new Set([
    ...interactions.map((r: any) => excludeKey(r.tmdb_id, r.media_type)),
    ...(watchlist || []).map((r: any) => excludeKey(r.tmdb_id, r.media_type)),
  ]);
  ...
  if (!profile) return { new_user: true }; // no rated title had usable DNA
  ...
  if (scored.length === 0) return { new_user: false, sections: {} };
  ...
  const lovedTitles = interactions.filter((i: any) => i.rating === 'love');
